...
The upload endpoint also accepts one `.xlsx` or `.xlsm` workbook at a time. Each worksheet is converted to a separate generated `.csv` file in the selected upload space, then each generated CSV is passed through the same analytics loader. Upload either multiple CSV files or one Excel workbook; mixing Excel with other files, or uploading multiple Excel workbooks in one request, is rejected.
